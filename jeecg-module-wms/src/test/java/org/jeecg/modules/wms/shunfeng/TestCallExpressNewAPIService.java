...
public class TestCallExpressNewAPIService {
     ...
     **/
    private static final String CLIENT_CODE = "YO8I8BSE";  //此处替换为您在丰桥平台获取的顾客编码
    private static final String CHECK_WORD = "nSv0pzcTHz4Faqk71TlREkjqVM9r6Udu";//此处替换为您在丰桥平台获取的校验码
...
//        String msgData =tools.packageMsgData(standardService);
        //找到下载sdk下json文件的路径
        byte[] bytes = IoUtil.readBytes(new FileInputStream("D:\\SF-CSIM-EXPRESS-SDK-V2.1.7\\json\\ExpressJson\\01.order.json"));
//        byte[] bytes = IoUtil.readBytes(new FileInputStream("D:\\course\\wms\\code\\02.order.query.json"));
        //byte[] bytes = IoUtil.readBytes(new FileInputStream("D:\\SF-CSIM-EXPRESS-SDK-V2.1.7\\json\\ExpressJson\\05.order.json"));
//        byte[] bytes = IoUtil.readBytes(new FileInputStream("D:\\course\\wms\\资料\\SF-CSIM-EXPRESS-SDK-V2.1.7-20220224\\SF-CSIM-EXPRESS-SDK-V2.1.7\\json\\ExpressJson\\20.cloud_print_waybills_2.json"));
        ...
        System.out.println("====调用丰桥的接口服务代码：" + String.valueOf("EXP_RECE_CREATE_ORDER") + " 接口耗时：" + String.valueOf(System.currentTimeMillis() - startTime) + "====");
