...
public class SISQWaveStrategy extends IWaveStrategy {
    ...
    void process(List<WmsOutOrders> orders, Map<String, List<WmsOutOrdersAllocation>> allocationsMap) {
        ...
        //找到符合单品单件的订单
        List<WmsOutOrders> matchedOrders = orders.stream().filter(order -> order.getTotalSku() == 1 && order.getTotalQuantity() == 1).collect(Collectors.toList());
        ...
        //最小订单数
        Integer minOrderCount = strategyByCode.getMinOrderCount();
        //出库单id
        ...
        //遍历matchedOrders,从allocations中找到对应的出库单明细并找到skuId，赋值到matchedOrders的对象中
        ...
        //剩余的订单
        List<WmsOutOrders> collect = orders.stream().filter(order -> !wmsOutOrders.contains(order)).collect(Collectors.toList());
        //执行下一个
