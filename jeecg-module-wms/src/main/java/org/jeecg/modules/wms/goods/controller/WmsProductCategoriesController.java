...
public class WmsProductCategoriesController extends JeecgController<WmsProductCategories, IWmsProductCategoriesService> {
    ...
    public Result<IPage<WmsProductCategories>> queryPageList(WmsProductCategories wmsProductCategories,
        ...
        if (hasQuery != null && "true".equals(hasQuery)) {
            ...
            //回填下级父节点
            wmsProductCategoriesService.fillParentName(pageList.getRecords());
            return Result.OK(pageList);
        }
    }
    ...
    public Result<IPage<WmsProductCategories>> queryPageList(WmsProductCategories wmsProductCategories, HttpServletRequest req) {
        ...
        IPage<WmsProductCategories> pageList = new Page<>(1, 10, list.size());
        pageList.setRecords(list);
        wmsProductCategoriesService.fillParentName(list);
        return Result.OK(pageList);
    }
     ...
     * @return
     */
    @AutoLog(value = "商品类别-启用")
    @Operation(summary = "商品类别-启用")
    ...
    public Result<String> enable(@RequestParam(name = "id", required = true) String id) {
        wmsProductCategoriesService.enable(id);
     ...
     * @return
     */
    @AutoLog(value = "商品类别-禁用")
    @Operation(summary = "商品类别-禁用")
    ...
    public Result<String> disable(@RequestParam(name = "id", required = true) String id) {
        wmsProductCategoriesService.disable(id);
